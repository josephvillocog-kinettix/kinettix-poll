...
const PollDetailView: React.FC<{ pollId: string; onBack: () => void; showBackButton: boolean; currentUser: User; }> = ({ pollId, onBack, showBackButton, currentUser }) => {
  ...
  const VoteConfirmationCard = (
    <div className="flex flex-col items-center justify-center text-center p-8 z-10 relative mt-8 sm:mt-16">
      <div className="bg-gray-800/60 backdrop-blur-xl border border-indigo-500/30 rounded-2xl shadow-2xl p-8 sm:p-12 max-w-lg mx-auto shadow-[0_0_45px_rgba(99,102,241,0.15)]">
          <div className="text-green-400 mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 sm:h-20 sm:w-20 mx-auto animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
          </div>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white">Thank you:</h2>
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white">{currentUser.name}!</h2>
          <p className="mt-4 text-md sm:text-lg text-gray-300">
              Please enjoy the night and have fun!
          </p>
      </div>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 z-10 relative">
      <div className="text-center mb-12 relative">
        ...
        <p className="mt-4 text-lg text-gray-300">
          {poll.status === 'open' && !hasVoted ? 'Select a candidate to cast your vote.' : hasVoted ? 'Your vote has been recorded.' : 'Viewing candidates for this poll.'}
        </p>
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden flex flex-col">
      ...
      {poll.status === 'open' && !hasVoted && voteFooter}
    </div>
  );
};
