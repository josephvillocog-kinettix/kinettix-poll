...
import { getActivePolls, getPollById } from '../services/votingService';
...
const PollDetailView: React.FC<{ pollId: string; onBack: () => void; showBackButton: boolean; }> = ({ pollId, onBack, showBackButton }) => {
    ...
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 z-10 relative">
        <div className="text-center mb-12 relative">
          ...
          <p className="mt-4 text-lg text-gray-300">Viewing candidates for this poll.</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            {filteredCandidates.map((candidate, index) => (
              <CandidateCard
                ...
                candidate={candidate}
                isSelected={false}
                disabled={true} // Cards are not selectable
                index={index}
              />
            ))}
          </div>
...
const VotingPage: React.FC<VotingPageProps> = ({ currentUser }) => {
  ...
  if (selectedPollId) {
    return <PollDetailView pollId={selectedPollId} onBack={handleBackToList} showBackButton={activePolls.length > 1} />;
  }
